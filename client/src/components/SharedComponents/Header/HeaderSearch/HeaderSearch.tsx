...
import {IBaseSearchItem} from "../../../../domain/entities";

export const HeaderSearch = () => {
    const {search, goToFileFromSearch} = useSearch();

    const [searchQuery, setSearchQuery] = React.useState("");
    const [searchData, setSearchData] = React.useState<IBaseSearchItem[]>([]);
    ...
    const clickHandler = React.useCallback((item: IBaseSearchItem) => {
    ...
    const fileImage = React.useCallback((item: IBaseSearchItem) => {
        const isFile = (getFileExtension(item.fileName));
